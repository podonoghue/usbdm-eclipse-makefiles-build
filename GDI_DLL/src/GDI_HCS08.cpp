...
DiReturnT DiRegisterWrite ( DiUInt32T        dnRegNumber,
   ...
   log.print("%s (%d) <= 0x%08X\n", getRegName(T_HCS08, dnRegNumber), dnRegNumber, (uint32_t)value);
...
DiReturnT DiRegisterRead ( DiUInt32T         dnRegNumber,
   ...
   USBDM_ErrorCode rc = BDM_RC_OK;
   LOGGING;
   log.print("%s (%d)\n", getRegName(T_HCS08, dnRegNumber), dnRegNumber);
   ...
   *drvValue = (U32c)dataValue;
   log.print("%s (%d) => 0x%08lX\n", getRegName(T_HCS08, dnRegNumber), dnRegNumber, (unsigned long)dataValue);
   return setErrorState(DI_OK);
}
