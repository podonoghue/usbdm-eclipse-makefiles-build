...
// The following quick commands take a fixed operand (N=1-31,0=>32) as part of the opcode
                                                                   // Operand
#define JTAG_SHIFT_IN_Q(N)       ((uint8_t)((3<<5)|((N)&JTAG_NUM_BITS_MASK))) // #5=N     Shift in N bits (fill with TDI=0/1)
#define JTAG_SHIFT_OUT_Q(N)      ((uint8_t)((4<<5)|((N)&JTAG_NUM_BITS_MASK))) // #5=N     Shift out N bits (data taken in-line)
#define JTAG_SHIFT_IN_OUT_Q(N)   ((uint8_t)((5<<5)|((N)&JTAG_NUM_BITS_MASK))) // #5=N     Shift out & in N bits (data taken in-line)
