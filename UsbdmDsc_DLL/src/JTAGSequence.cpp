...
USBDM_ErrorCode executeJTAGSequence(uint8_t        sequenceLength,
...
#else
   if (doLog) {
      UsbdmSystem::Log::print("executeJTAGSequence()=>\n");
   ...
   if (doLog) {
      if (rc == BDM_RC_OK) {
         UsbdmSystem::Log::print("Returned data =>    ----------------------------------------\n");
         UsbdmSystem::Log::printDump(dataInStart, dataInLength);
      }
      UsbdmSystem::Log::print("==============================================================\n");
   }
#endif
   if (rc != BDM_RC_OK) {
      UsbdmSystem::Log::print("Failed, reason = %s\n", USBDM_GetErrorString(rc));
   }
   return rc;
}
...
//               case JTAG_SHIFT_IN_VARB:
               ...
               default:
                  log.print("Unhandled opcode = %d, SP=%d\n", opcode, sequence-sequenceStart);
                  break;
...
//            log.print("JTAG_SHIFT_OUT_Q(%d) SP=%d\n", numBits, sequence-sequenceStart);
...
USBDM_ErrorCode executeTargetInstructionSequence(const uint8_t **dataOutPtr) {
   ...
   do {
      ...
      log.print("(#%d) : ", numberOfInstructions);
      do {
         uint16_t opcode = ((**dataOutPtr)<<8) + (*(*dataOutPtr+1));
         if (numWords>1) {
            log.print("0x%4.4X, ", opcode);
...
int                  indent            = 6;
   ...
   UsbdmSystem::Log::print("(seqLength = %d, sequence=%d, dataOutPtr=%d, dataInPtr=%d) \n",
         sequenceLength,
   ...
   do {
      ...
      if ((sequence>=sequenceStart) && (sequence<(sequenceStart+sequenceLength)))
         ...
         UsbdmSystem::Log::print("%-*d: Illegal sequence ptr = %d\n", indent, lineNumber, sequence-sequenceStart);
      ...
      if (dataOutPtr>sequenceStart+sequenceLength) {
         UsbdmSystem::Log::print("%-*d: Illegal dataOut ptr = %d\n", indent, lineNumber, sequence-sequenceStart);
         rc = BDM_RC_JTAG_ILLEGAL_SEQUENCE;
      }
      ...
      switch (opcode&JTAG_COMMAND_MASK) {
         ...
         case JTAG_MISC1: // Misc commands
            switch (opcode) {
               ...
               case JTAG_SUBC:
               case JTAG_SUBD:
                  UsbdmSystem::Log::print("%-*d: JTAG_SUB%c - start addr=%d\n", indent,
                  ...
                  break;
               case JTAG_CALL_SUBA:
                  UsbdmSystem::Log::print("%-*d: JTAG_CALL_SUBA*\n", indent, lineNumber);
               ...
               case JTAG_CALL_SUBD:
                  ...
                  UsbdmSystem::Log::print("%-*d: JTAG_CALL_SUB%c, call addr=%d, dataOutPtr = %d(0x%X), iterator = %d\n", indent,
                        lineNumber, 'A'+(opcode&0x03), sequence-sequenceStart,
                        dataOutPtr-sequenceStart, dataOutPtr-sequenceStart, iterator);
                  UsbdmSystem::Log::print("%-*d: repeatTOS                = %p\n", indent, lineNumber, repeatTOS);
                  UsbdmSystem::Log::print("%-*d: (subroutineTOS-1)->repeatTOS = %p\n", indent, lineNumber, (subroutineTOS-1)->repeatTOS);
                  UsbdmSystem::Log::print("%-*d: repeatStack              = %p\n", indent, lineNumber, repeatStack);
                  UsbdmSystem::Log::print("%-*d: subroutineTOS            = %p\n", indent, lineNumber, subroutineTOS);
                  UsbdmSystem::Log::print("%-*d: subroutineStack          = %p\n", indent, lineNumber, subroutineStack);
                  indent+=3;
                  break;
               case JTAG_BREAK:
                  iterator = 1;
               ...
               case JTAG_END_REPEAT:
                  ...
                  if (((subroutineTOS != subroutineStack) && (repeatTOS == (subroutineTOS-1)->repeatTOS )) ||
                     ...
                     UsbdmSystem::Log::print("%-*d: JTAG_END_REPEAT unexpected\n", indent, lineNumber);
                     UsbdmSystem::Log::print("%-*d: repeatTOS                = %p\n", indent, lineNumber, repeatTOS);
                     UsbdmSystem::Log::print("%-*d: subroutineTOS->repeatTOS = %p\n", indent, lineNumber, subroutineTOS->repeatTOS);
                     UsbdmSystem::Log::print("%-*d: repeatStack              = %p\n", indent, lineNumber, repeatStack);
                     UsbdmSystem::Log::print("%-*d: subroutineTOS            = %p\n", indent, lineNumber, subroutineTOS);
                     UsbdmSystem::Log::print("%-*d: subroutineStack          = %p\n", indent, lineNumber, subroutineStack);
                  }
                     ...
                     indent-=3;
                     UsbdmSystem::Log::print("%-*d: JTAG_END_REPEAT leaving loop\n", indent, lineNumber);
                  }
                  else {
                     sequence = startOfIteration;
                     UsbdmSystem::Log::print("%-*d: JTAG_END_REPEAT #%d, addr=%d\n", indent, lineNumber, iterator, sequence-sequenceStart);
                  }
                  break;
               ...
               case JTAG_SKIP_DP:   // Skip over N bytes in data Out sequence
                  UsbdmSystem::Log::print("%-*d: JTAG_SKIP_DP(%d)", indent, lineNumber, tempValue);
                  dataOutPtr += tempValue;
                  UsbdmSystem::Log::print(" New DP=%d\n",dataOutPtr-sequenceStart);
                  break;
               case JTAG_END:
                  indent-=3;
               ...
               case JTAG_END_SUB:
                  ...
                  if (subroutineTOS == subroutineStack) {
                     UsbdmSystem::Log::print("%-*d: JTAG_END_SUB/RETURN - stack underflow, DP=%d(0x%X)\n", indent,
                     ...
                     UsbdmSystem::Log::print("%-*d: JTAG_END_SUB/RETURN - returning, return addr=%d, DP=%d(0x%X)\n", indent,
                           lineNumber, sequence-sequenceStart,
               ...
               case JTAG_SHIFT_OUT_DP_VARA:
                  ...
                  if (dataOutPtr == NULL) {
                     ...
                     rc = BDM_RC_JTAG_ILLEGAL_SEQUENCE;
                     UsbdmSystem::Log::print("%-*d: JTAG_SHIFT_OUT_DP_VARA(N) No output buffer\n", indent, lineNumber);
                  }
                  else if (numBits == 0) {
                     rc = BDM_RC_JTAG_ILLEGAL_SEQUENCE;
                     UsbdmSystem::Log::print("%-*d: JTAG_SHIFT_OUT_DP_VARA(0) - Illegal repeat count!\n", indent, lineNumber);
                  }
                  else {
                     UsbdmSystem::Log::print("%-*d: JTAG_SHIFT_OUT_DP_VARA (=%d) => ", indent, lineNumber, numBits);
               ...
               case JTAG_SHIFT_OUT_DP : // Shift sequence out - sequence taken from dataOutPtr
                  ...
                  if (dataOutPtr == NULL) {
                     // Requires data out ptr.
                     rc = BDM_RC_JTAG_ILLEGAL_SEQUENCE;
                     UsbdmSystem::Log::print("%-*d: JTAG_SHIFT_OUT_DP(N) No output buffer\n", indent, lineNumber);
                  }
                  else if (numBits == 0) {
                     rc = BDM_RC_JTAG_ILLEGAL_SEQUENCE;
                     UsbdmSystem::Log::print("%-*d: JTAG_SHIFT_OUT_DP(0) - Illegal repeat count!\n", indent, lineNumber);
                  }
                  else {
                     UsbdmSystem::Log::print("%-*d: JTAG_SHIFT_OUT_DP (%d) => ", indent, lineNumber, numBits);
                     printBits(numBits, dataOutPtr);
                     rc = USBDM_JTAG_Write(numBits, exitAction, dataOutPtr);
                     dataOutPtr += BITS_TO_BYTES(numBits);
                  }
               ...
               case JTAG_SHIFT_IN_DP : // Shift sequence in (8-bit count)/TL
                  ...
                  if (dataInPtr == NULL) {
                     rc = BDM_RC_JTAG_ILLEGAL_SEQUENCE;
                     UsbdmSystem::Log::print("%-*d: JTAG_SHIFT_IN_TL(N) - No input buffer!\n", indent, lineNumber);
                  }
                  else if (numBits == 0) {
                     rc = BDM_RC_JTAG_ILLEGAL_SEQUENCE;
                     UsbdmSystem::Log::print("%-*d: JTAG_SHIFT_IN_TL(0) - Illegal repeat count!\n", indent, lineNumber);
                  }
                  else {
                     rc = USBDM_JTAG_Read(numBits, exitAction|inFill, dataInPtr);
                     UsbdmSystem::Log::print("%-*d: JTAG_SHIFT_IN_TL (%d) <= ", indent, lineNumber, numBits);
               ...
               case JTAG_SHIFT_IN_OUT_DP : // Shift sequence in & out at same time - sequence taken from dataOutPtr
                  ...
                  if (dataOutPtr == NULL) {
                     // Requires data out ptr.
                     rc = BDM_RC_JTAG_ILLEGAL_SEQUENCE;
                     UsbdmSystem::Log::print("%-*d: JTAG_SHIFT_IN_OUT_TL(N) No output buffer\n", indent, lineNumber);
                  }
                  else if (dataInPtr == NULL) {
                     rc = BDM_RC_JTAG_ILLEGAL_SEQUENCE;
                     UsbdmSystem::Log::print("%-*d: JTAG_SHIFT_IN_OUT_TL(N) - No input buffer!\n", indent, lineNumber);
                  }
                  else if (numBits == 0) {
                     rc = BDM_RC_JTAG_ILLEGAL_SEQUENCE;
                     UsbdmSystem::Log::print("%-*d: JTAG_SHIFT_IN_OUT_TL(0) - Illegal repeat count!\n", indent, lineNumber);
                  }
                  else {
                     UsbdmSystem::Log::print("%-*d: JTAG_SHIFT_IN_OUT_TL (%d) => ", indent, lineNumber, numBits);
                     printBits(numBits, dataOutPtr);
                     rc = USBDM_JTAG_ReadWrite(numBits, exitAction, dataOutPtr, dataInPtr);
                     UsbdmSystem::Log::print("%-*d:                      (%d) <= ", indent, lineNumber, numBits);
                     printBits(numBits, dataInPtr);
                     dataOutPtr     += BITS_TO_BYTES(numBits);
                     dataInPtr      += BITS_TO_BYTES(numBits);
                  }
               ...
               case JTAG_IF_VARB_NEQ:
                  UsbdmSystem::Log::print("%-*d: JTAG_IF_VAR%c_NEQ var=%d, value=%d\n", indent, lineNumber, 'A'+(opcode&3), (uint32_t)variables[opcode&3], (uint32_t)tempValue);
                  indent+=3;
                  ...
                  break;
               case JTAG_IF_ITER_EQ:
                  UsbdmSystem::Log::print("%-*d: JTAG_IF_ITER_EQ iterator=%d, value=%d\n", indent, lineNumber, iterator, (uint32_t)tempValue);
                  indent+=3;
                  ...
                  break;
               case JTAG_IF_ITER_NEQ:
                  UsbdmSystem::Log::print("%-*d: JTAG_IF_ITER_NEQ iterator=%d, value=%d\n", indent, lineNumber, iterator, (uint32_t)tempValue);
                  indent+=3;
                  if (iterator == tempValue) { // Fail => skip to ELSE clause
                     sequence = skipSequence(sequence, STOP_ON_ELSE|STOP_ON_END_IF|STOP_ON_SUB, &indent);
                  }
               ...
               case JTAG_REPEAT:
                  if (tempValue == 0) {
                     rc = BDM_RC_JTAG_ILLEGAL_SEQUENCE;
                     UsbdmSystem::Log::print("%-*d: JTAG_REPEAT(%d) - Illegal repeat count!\n", indent, lineNumber, tempValue);
                  }
                  else {
                     repeatTOS->iterator    = iterator;
                     repeatTOS->startOfLoop = startOfIteration;
...
//                     rc = BDM_RC_JTAG_ILLEGAL_SEQUENCE;
//                     UsbdmSystem::Log::print("%-*d: JTAG_SHIFT_IN_VAR%c(%d) - Illegal repeat count!\n", indent, lineNumber, 'A'+(opcode&0x3), numBits);
//                  }
//                  else {
//                     JTAG32 temp(0,32);
//                     rc = USBDM_JTAG_Read(numBits, exitAction|inFill, temp.getData(numBits));
//                     UsbdmSystem::Log::print("%-*d: JTAG_SHIFT_IN_VAR%c (%d) varA <= %s", indent, lineNumber, 'A'+(opcode&0x3), numBits);
...
//                  break;
               ...
               case JTAG_SHIFT_OUT_VARD:
                  ...
                  if ((numBits == 0) || (numBits > 32)) {
                     rc = BDM_RC_JTAG_ILLEGAL_SEQUENCE;
                     UsbdmSystem::Log::print("%-*d: JTAG_SHIFT_OUT_VAR%c(%d) - Illegal repeat count!\n", indent, lineNumber, 'A'+(opcode&0x3), numBits);
                  }
                  else {
                     JTAG32 temp(variables[opcode&0x3],32);
                     UsbdmSystem::Log::print("%-*d: JTAG_SHIFT_OUT_VAR%c(%d) => ", indent, lineNumber, 'A'+(opcode&0x3), numBits);
               ...
               case JTAG_SHIFT_IN_OUT_VARD:
                  ...
                  if ((numBits == 0) || (numBits > 32)) {
                     rc = BDM_RC_JTAG_ILLEGAL_SEQUENCE;
                     UsbdmSystem::Log::print("%-*d: JTAG_SHIFT_IN_OUT_VAR%c(%d) - Illegal repeat count!\n", indent, lineNumber, 'A'+(opcode&0x3), numBits);
                  }
                  else {
                     JTAG32 temp(0,32);
                     UsbdmSystem::Log::print("%-*d: JTAG_SHIFT_IN_OUT_VAR%c(%d) => ", indent, lineNumber, 'A'+(opcode&0x3), numBits);
                     printBits(numBits, sequence);
               ...
               case JTAG_PUSH8:
                  tempValue = *sequence++;
                  if (tempValue<32) {
                     UsbdmSystem::Log::print("%-*d: JTAG_PUSH_8(%d, 0x%2.2X) - Warning - Use JTAG_PUSH_Q()\n", indent, lineNumber, (uint32_t)tempValue, (uint32_t)tempValue);
                  }
                  else
               ...
               case JTAG_PUSH16:
                  ...
                  if (tempValue<=0xFFFFUL) {
                     UsbdmSystem::Log::print("%-*d: JTAG_PUSH_16(%d, 0x%4.4X) - Warning - Use JTAG_PUSH_Q/8()\n", indent, lineNumber, (uint32_t)tempValue, (uint32_t)tempValue);
                  }
                  else
               ...
               case JTAG_PUSH32:
                  ...
                  tempValue = (tempValue<<8)+*sequence++;
                  if (tempValue<=0xFFFFUL) {
                     UsbdmSystem::Log::print("%-*d: JTAG_PUSH_32(%d, 0x%8.8X) - Warning - Use JTAG_PUSH_Q/8/16()\n", indent, lineNumber, (uint32_t)tempValue, (uint32_t)tempValue);
                  }
                  else
               ...
               case JTAG_PUSH_DP_32:
                  ...
                  tempValue = (tempValue<<8)+*dataOutPtr++;
                  tempValue = (tempValue<<8)+*dataOutPtr++;
                  UsbdmSystem::Log::print("%-*d: JTAG_PUSH_DP_32(dptr=%d, 0x%8.8X)\n", indent, lineNumber, dataOutPtr-sequenceStart, (uint32_t)tempValue);
                  break;
               case JTAG_SAVE_OUT_DP_VARC:
               case JTAG_SAVE_OUT_DP_VARD:
                  UsbdmSystem::Log::print("%-*d: JTAG_SAVE_DP_VAR%c DP=%d\n", indent, lineNumber, 'A'+(opcode&0x03), dataOutPtr-sequenceStart);
                  variables[regNo] = dataOutPtr-sequenceStart;
                  break;
               case JTAG_RESTORE_DP_VARC:
               case JTAG_RESTORE_DP_VARD:
                  dataOutPtr = variables[regNo]+sequenceStart;
                  UsbdmSystem::Log::print("%-*d: JTAG_RESTORE_DP_VAR%c DP=%d\n", indent, lineNumber, 'A'+(opcode&0x03), dataOutPtr-sequenceStart);
                  break;
               case JTAG_LOAD_VARA:
               case JTAG_LOAD_VARB:
         ...
         case JTAG_REPEAT_Q(0):
            ...
            if (iterator == 0) {
               rc = BDM_RC_JTAG_ILLEGAL_SEQUENCE;
               UsbdmSystem::Log::print("%-*d: JTAG_REPEAT_DP #0 - Illegal repeat count!\n", indent, lineNumber);
            }
            else {
               startOfIteration       = sequence;
            }
         ...
         case JTAG_SHIFT_IN_Q(0) : // Shift sequence in (5-bit count)
            if (dataInPtr == NULL) {
               rc = BDM_RC_JTAG_ILLEGAL_SEQUENCE;
               UsbdmSystem::Log::print("%-*d: JTAG_SHIFT_IN_Q(%d) - no input buffer!\n", indent, lineNumber, numBits);
            }
            else {
               rc = USBDM_JTAG_Read(numBits, exitAction|inFill, dataInPtr);
               UsbdmSystem::Log::print("%-*d: JTAG_SHIFT_IN_Q(%d) <= ", indent, lineNumber, numBits);
               printBits(numBits, dataInPtr);
               dataInPtr   += BITS_TO_BYTES(numBits);
            }
            break;
         ...
         case JTAG_SHIFT_IN_OUT_Q(0) : // Shift sequence in & out at same time - sequence taken inline
            if (dataInPtr == NULL) {
               rc = BDM_RC_JTAG_ILLEGAL_SEQUENCE;
               UsbdmSystem::Log::print("%-*d: JTAG_SHIFT_IN_OUT_Q(%d) - no input buffer!\n", indent, lineNumber, numBits);
            }
            else {
      ...
      if (dataInPtr > dataInStart+dataInLength) {    // Input buffer overflow
         UsbdmSystem::Log::print("%-*d: Illegal dataIn ptr = %d\n", indent, lineNumber, dataOutPtr-sequenceStart);
         rc = BDM_RC_JTAG_ILLEGAL_SEQUENCE;
      }
      if (indent>30)
         indent -= 30;
   ...
   if ((dataInPtr-dataInStart) != dataInLength) {
      rc = BDM_RC_JTAG_ILLEGAL_SEQUENCE;
      UsbdmSystem::Log::print("executeJTAGSequence(inLength=%d), input length mismatch, actual=%d)\n", dataInLength, dataInPtr-dataInStart);
   }
   if (rc != BDM_RC_OK) {
      UsbdmSystem::Log::print("executeJTAGSequence() - error, reason = %s\n", USBDM_GetErrorString(rc));
   }
   return rc;
}
...
// UsbdmSystem::Log::print("Input Data --------------------------------------\n");
...
//                  break;
               ...
               case JTAG_SET_PADDING:  // #Set HDR, HIR, TDR, TIR
                  ...
                  break;
               case JTAG_READ_MEM:
                  UsbdmSystem::Log::print("%-*d JTAG_READ_MEM...\n", indent, lineNumber);
                  break;
               case JTAG_WRITE_MEM:
                  UsbdmSystem::Log::print("%-*d JTAG_WRITE_MEM...\n", indent, lineNumber);
                  break;
               default:
                  UsbdmSystem::Log::print("%-*d Unknown Opcode (%d)\n", indent, lineNumber, opcode);
                  break;
