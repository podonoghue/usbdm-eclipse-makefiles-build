...
//!
USBDM_ErrorCode FlashProgrammer_CFVx::readTargetChipId(uint32_t *targetSDID) {
   LOGGING_E;
...
//! +---------------------------------------------------+ -+
//!
USBDM_ErrorCode FlashProgrammer_CFVx::loadLargeTargetProgram(uint8_t    *buffer,
                                                        uint32_t              loadAddress,
                                                        uint32_t              size,
...
//! +-----------------------------------------+ -+
//!
USBDM_ErrorCode FlashProgrammer_CFVx::loadSmallTargetProgram(uint8_t    *buffer,
                                                        uint32_t              loadAddress,
                                                        uint32_t              size,
                                                        FlashProgramConstPtr  flashProgram,
                                                        FlashOperation        flashOperation) {
...
//! @note - Assumes flash programming code has already been loaded to target.
//!
USBDM_ErrorCode FlashProgrammer_CFVx::partitionFlexNVM() {
   LOGGING;
...
//!
USBDM_ErrorCode FlashProgrammer_CFVx::checkUnsupportedTarget() {
   LOGGING;
   USBDM_ErrorCode rc;
   uint32_t targetSDID;
...
//!
USBDM_ErrorCode FlashProgrammer_CFVx::getTargetBusSpeed(unsigned long *busFrequency) {
   LOGGING;
...
//!
void FlashProgrammer_CFVx::deleteSecurityAreas(void) {
   securityAreaCount = 0;
}
...
//! @return error code see \ref USBDM_ErrorCode.
//!
USBDM_ErrorCode FlashProgrammer_CFVx::recordSecurityArea(const uint32_t address, const int size, const uint8_t *data) {
...
//!
void FlashProgrammer_CFVx::restoreSecurityAreas(FlashImagePtr flashImage) {
   LOGGING_Q;
...
//!
USBDM_ErrorCode FlashProgrammer_CFVx::setFlashSecurity(FlashImagePtr flashImage, MemoryRegionConstPtr flashRegion) {
   LOGGING;
...
//! @return error code see \ref USBDM_ErrorCode.
//!
USBDM_ErrorCode FlashProgrammer_CFVx::setFlashSecurity(FlashImagePtr flashImage) {
   LOGGING;
   // Process each flash region
   USBDM_ErrorCode rc = BDM_RC_OK;
...
//!
USBDM_ErrorCode FlashProgrammer_CFVx::doFlashBlock(FlashImagePtr flashImage,
...
extern "C" FlashProgrammer USBDM_FLASHPROGRAMMER_DECLSPEC *createPluginInstance() {
   return new FlashProgrammer_CFVx();
}
