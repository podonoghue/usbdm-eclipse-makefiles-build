...
//!
USBDM_ErrorCode FlashProgrammer_HCS12::readTargetChipId(uint32_t *targetSDID) {
   LOGGING_E;
...
//! @return error code, see \ref USBDM_ErrorCode
//!
USBDM_ErrorCode FlashProgrammer_HCS12::massEraseTarget(void) {
   LOGGING;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS12::probeMemory(MemorySpace_t memorySpace, uint32_t address) {
   LOGGING_Q;
   ...
   if (rc != BDM_RC_OK) {
      log.error("RAM memory probe failed @0x%08X, rc = %d (%s)\n", address, rc, bdmInterface->getErrorString(rc));
      return rc;
   }
   return PROGRAMMING_RC_OK;
}
...
//!
void FlashProgrammer_HCS12::deleteSecurityAreas(void) {
   securityAreaCount = 0;
}
...
//! @return error code see \ref USBDM_ErrorCode.
//!
USBDM_ErrorCode FlashProgrammer_HCS12::recordSecurityArea(const uint32_t address, const int size, const uint8_t *data) {
...
//!
void FlashProgrammer_HCS12::restoreSecurityAreas(FlashImagePtr flashImage) {
   LOGGING_Q;
...
//! @return error code see \ref USBDM_ErrorCode
//!
USBDM_ErrorCode FlashProgrammer_HCS12::setDeviceData(const DeviceData &theParameters) {
   LOGGING_Q;
   currentFlashProgram.reset();
