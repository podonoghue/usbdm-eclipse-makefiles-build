...
ModuleInfo FlashProgrammer_HCS08::moduleInfo;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS08::getPageAddress(MemoryRegionConstPtr memoryRegionPtr, uint32_t physicalAddress, uint8_t *pageNo) {
   LOGGING_Q;
   *pageNo = 0x00;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS08::setPageRegisters(uint32_t physicalAddress) {
   LOGGING_Q;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS08::resetAndConnectTarget(void) {
   LOGGING;
   USBDM_ErrorCode rc;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS08::readTargetChipId(uint32_t *targetSDID, bool doInit) {
   LOGGING_E;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS08::confirmSDID() {
   LOGGING_E;
   uint32_t targetSDID;
   USBDM_ErrorCode rc;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS08::initialiseTarget() {
   LOGGING;
   USBDM_ErrorCode rc;
   if (initTargetDone) {
...
//! @note Assumes target has been reset & connected
//!
USBDM_ErrorCode FlashProgrammer_HCS08::initialiseTargetFlash() {
   LOGGING;
   USBDM_ErrorCode rc;
 ...
 *  @return error code, see \ref USBDM_ErrorCode
 */
USBDM_ErrorCode FlashProgrammer_HCS08::massEraseTarget(bool resetTarget) {
   LOGGING;
...
//! @note - see loadTargetProgram(MemoryRegionConstPtr, FlashOperation) for details
//!
USBDM_ErrorCode FlashProgrammer_HCS08::loadTargetProgram(MemoryRegionConstPtr memoryRegionPtr, FlashOperation flashOperation) {
   LOGGING_Q;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS08::loadTargetProgram(FlashProgramConstPtr flashProgram, FlashOperation flashOperation) {
   LOGGING;
...
//! +---------------------------------------------------+ -+
//!
USBDM_ErrorCode FlashProgrammer_HCS08::loadLargeTargetProgram(
                                                        uint8_t    *buffer,
                                                        uint32_t              imageAddress,
                                                        uint32_t              imageSize,
   ...
   // First 2 bytes are address of header in target memory
   ...
   MemorySpace_t memorySpace = MS_Byte;
#endif
   headerPtr->loadAddress   = nativeToTarget16(codeLoadAddress);
...
USBDM_ErrorCode FlashProgrammer_HCS08::loadSmallTargetProgram(
   ...
   log.print("Loaded Image (unmodified) :\n");
   log.print("   flashProgramHeader program         = 0x%08X\n",     program);
   log.print("   flashProgramHeader.massErase       = 0x%08X\n",     massErase);
   log.print("   flashProgramHeader.blankCheck      = 0x%08X\n",     blankCheck);
   log.print("   flashProgramHeader.selectiveErase  = 0x%08X\n",     selectiveErase);
   log.print("   flashProgramHeader.verify          = 0x%08X\n",     verify);
   log.print("   flashProgramHeader.loadAddress     = 0x%08X\n",     loadAddress);
   ...
   case OpWriteRam:
   default:
      log.error("flashOperation is invalid.\n");
      return PROGRAMMING_RC_ERROR_ILLEGAL_PARAMS;
   }
   if (codeStart <= 0) {
      log.error("codeStart address is invalid.\n");
      return PROGRAMMING_RC_ERROR_ILLEGAL_PARAMS;
   }
   if (codeEnd <= 0) {
      codeEnd = size;
...
//   targetProgramInfo.usePagedAddresses = (program>=sizeof(SmallTargetFlashDataHeader));
   ...
   log.print("Parameters[0x%04X...0x%04X]\n",
   ...
   if (targetProgramInfo.maxDataSize<40) {
      log.print("Data buffer is too small - 0x%X \n", targetProgramInfo.maxDataSize);
      return PROGRAMMING_RC_ERROR_INTERNAL_CHECK_FAILED;
   }
#if TARGET == MC56F80xx
   MemorySpace_t memorySpace = MS_PWord;
...
#endif
   // Write the flash programming code to target memory
   if (bdmInterface->writeMemory(memorySpace, codeLoadSize, codeLoadAddress, buffer+codeStart) != BDM_RC_OK) {
      return PROGRAMMING_RC_ERROR_BDM_WRITE;
   }
   currentFlashProgram   = flashProgram;
   currentFlashOperation = flashOperation;
...
//!
const char *FlashProgrammer_HCS08::getProgramActionNames(unsigned int actions) {
unsigned index;
...
//!
const char *FlashProgrammer_HCS08::getProgramCapabilityNames(unsigned int actions) {
   unsigned index;
   static char buff[250] = "";
   static const char *actionTable[] = {
...
USBDM_ErrorCode FlashProgrammer_HCS08::convertTargetErrorCode(FlashDriverError_t rc) {
...
USBDM_ErrorCode FlashProgrammer_HCS08::initLargeTargetBuffer(uint8_t *buffer) {
   ...
   pFlashHeader->watchdogAddress = nativeToTarget16(device->getWatchdogAddress());
...
USBDM_ErrorCode FlashProgrammer_HCS08::initSmallTargetBuffer(uint8_t *buffer) {
   LOGGING;
   ...
   if (targetProgramInfo.useWatchdogAddress && targetProgramInfo.usePagedAddresses) {
      log.error("Error: Small flash code doesn't support watchdog+paged addresses\n");
      return PROGRAMMING_RC_ERROR_INTERNAL_CHECK_FAILED;
   }
   if (targetProgramInfo.useWatchdogAddress) {
      log.print("Setting watchdogAddress\n");
      pFlashHeader->page_wdog_Address = nativeToTarget16(device->getWatchdogAddress());
   }
   if (targetProgramInfo.usePagedAddresses) {
      log.print("Setting pageAddress\n");
      ...
      log.print("FlashProgrammer_HCS08::initSmallTargetBuffer() - Error: Paged address not supported\n");
      return PROGRAMMING_RC_ERROR_INTERNAL_CHECK_FAILED;
   }
   switch(currentFlashOperation) {
   case OpNone:
   default:
      log.error("Error: Unsupported operation\n");
      return PROGRAMMING_RC_ERROR_INTERNAL_CHECK_FAILED;
   case OpSelectiveErase:
   {
...
//!
USBDM_ErrorCode FlashProgrammer_HCS08::executeTargetProgram(uint8_t *pBuffer, uint32_t dataSize) {
   LOGGING;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS08::determineTargetSpeed(void) {
   LOGGING;
...
//! @return error code see \ref USBDM_ErrorCode.
//!
USBDM_ErrorCode FlashProgrammer_HCS08::eraseFlash(void) {
   LOGGING;
   USBDM_ErrorCode rc = BDM_RC_OK;
...
//! @return error code see \ref USBDM_ErrorCode.
//!
USBDM_ErrorCode FlashProgrammer_HCS08::selectiveEraseFlashSecurity(void) {
   LOGGING;
   USBDM_ErrorCode rc = BDM_RC_OK;
...
//! @note - Assumes flash programming code has already been loaded to target.
//!
USBDM_ErrorCode FlashProgrammer_HCS08::partitionFlexNVM() {
   LOGGING;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS08::getTargetBusSpeed(unsigned long *busFrequency) {
   LOGGING;
...
//! @note Assumes the target device has already been opened & USBDM options set.
//!
USBDM_ErrorCode FlashProgrammer_HCS08::checkTargetUnSecured() {
   LOGGING;
   USBDM_ErrorCode rc = initialiseTarget();
   if (rc != PROGRAMMING_RC_OK)
      return rc;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS08::setFlashSecurity(FlashImagePtr flashImage, MemoryRegionConstPtr flashRegion) {
   LOGGING;
   ...
   flashImage->loadDataBytes(securityRegionSize, securityRegionAddress, securityData, false);
   ...
   if ((getEraseMethod() == DeviceData::eraseMass) || (getEraseMethod() == DeviceData::eraseNone)) {
      ...
      if (memcmp(securityData, memory, securityRegionSize) == 0) {
         ...
         // Force erase of security area in target before programming as it is non-blank and needs modification
         log.print("Security area in target needs erasing\n");
         securityNeedsSelectiveErase = true;
      }
      // else it is blank and doesn't need erasing
   }
   return PROGRAMMING_RC_OK;
}
...
//!
USBDM_ErrorCode FlashProgrammer_HCS08::setFlashSecurity(FlashImagePtr flashImage) {
   LOGGING;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS08::doFlashBlock(
...
//! @return error code see \ref USBDM_ErrorCode
//!
USBDM_ErrorCode FlashProgrammer_HCS08::applyFlashOperation(FlashImagePtr flashImage, FlashOperation  flashOperation) {
   LOGGING;
...
//! @return error code see \ref USBDM_ErrorCode
//!
USBDM_ErrorCode FlashProgrammer_HCS08::doProgram(FlashImagePtr flashImage) {
   LOGGING;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS08::doSelectiveErase(FlashImagePtr flashImage) {
   LOGGING;
...
USBDM_ErrorCode FlashProgrammer_HCS08::doImageCheck(FlashImagePtr flashImage) {
   LOGGING;
...
//!
USBDM_ErrorCode FlashProgrammer_HCS08::doReadbackVerify(FlashImagePtr flashImage) {
   LOGGING;
...
//! @return error code see \ref USBDM_ErrorCode
//!
USBDM_ErrorCode FlashProgrammer_HCS08::doTargetVerify(FlashImagePtr flashImage) {
   LOGGING;
...
//! @return error code see \ref USBDM_ErrorCode
//!
USBDM_ErrorCode FlashProgrammer_HCS08::doVerify(FlashImagePtr flashImage) {
   LOGGING;
   progressTimer->restart("Verifying...");
...
//! @return error code see \ref USBDM_ErrorCode
//!
USBDM_ErrorCode FlashProgrammer_HCS08::doBlankCheck(FlashImagePtr flashImage) {
   LOGGING;
...
//! @return error code see \ref USBDM_ErrorCode
//!
USBDM_ErrorCode FlashProgrammer_HCS08::doWriteRam(FlashImagePtr flashImage) {
   LOGGING;
...
//!       locations are ignored.
//!
USBDM_ErrorCode FlashProgrammer_HCS08::verifyFlash(FlashImagePtr flashImage,
   ...
   // Needed for blank check related to security
...
//!
USBDM_ErrorCode FlashProgrammer_HCS08::programFlash(FlashImagePtr flashImage,
                                              CallBackT    progressCallBack,
                                              bool         doRamWrites) {
   LOGGING;
 ...
 */
extern "C"
size_t CPP_DLL_EXPORT createPluginInstance(FlashProgrammer_HCS08 *pp) {
