...
#include "FlashProgrammer_CFV1.h"
...
//!
USBDM_ErrorCode FlashProgrammer_CFV1::getPageAddress(MemoryRegionConstPtr memoryRegionPtr, uint32_t physicalAddress, uint8_t *pageNo) {
   LOGGING_Q;
   *pageNo = 0x00;
...
//!
USBDM_ErrorCode FlashProgrammer_CFV1::setPageRegisters(uint32_t physicalAddress) {
   LOGGING_Q;
...
//!
USBDM_ErrorCode FlashProgrammer_CFV1::resetAndConnectTarget(void) {
   LOGGING;
   USBDM_ErrorCode rc;
...
#if (TARGET == ARM) || (TARGET == MC56F80xx) || (TARGET == CFVx) || (TARGET == HC12)
#else
...
//!
USBDM_ErrorCode FlashProgrammer_CFV1::readTargetChipId(uint32_t *targetSDID, bool doInit) {
   LOGGING_E;
...
//!
USBDM_ErrorCode FlashProgrammer_CFV1::confirmSDID() {
   LOGGING_E;
   uint32_t targetSDID;
   USBDM_ErrorCode rc;
...
//!
USBDM_ErrorCode FlashProgrammer_CFV1::initialiseTarget() {
   LOGGING;
   USBDM_ErrorCode rc;
   if (initTargetDone) {
...
//! @note Assumes target has been reset & connected
//!
USBDM_ErrorCode FlashProgrammer_CFV1::initialiseTargetFlash() {
   LOGGING;
   USBDM_ErrorCode rc;
...
//! @return error code, see \ref USBDM_ErrorCode
//!
USBDM_ErrorCode FlashProgrammer_CFV1::massEraseTarget(bool resetTarget) {
   LOGGING;
...
//!
USBDM_ErrorCode FlashProgrammer_CFV1::loadTargetProgram(FlashOperation flashOperation) {
   LOGGING;
...
//! @note - see loadTargetProgram(MemoryRegionConstPtr, FlashOperation) for details
//!
USBDM_ErrorCode FlashProgrammer_CFV1::loadTargetProgram(MemoryRegionConstPtr memoryRegionPtr, FlashOperation flashOperation) {
   LOGGING_Q;
...
//!
USBDM_ErrorCode FlashProgrammer_CFV1::loadTargetProgram(FlashProgramConstPtr flashProgram, FlashOperation flashOperation) {
   LOGGING;
...
//! +---------------------------------------------------+ -+
//!
USBDM_ErrorCode FlashProgrammer_CFV1::loadLargeTargetProgram(uint8_t    *buffer,
                                                        uint32_t              loadAddress,
                                                        uint32_t              size,
...
//! +-----------------------------------------+ -+
//!
USBDM_ErrorCode FlashProgrammer_CFV1::loadSmallTargetProgram(uint8_t    *buffer,
                                                        uint32_t              loadAddress,
                                                        uint32_t              size,
                                                        FlashProgramConstPtr  flashProgram,
                                                        FlashOperation        flashOperation) {
...
//!
const char *FlashProgrammer_CFV1::getProgramActionNames(unsigned int actions) {
unsigned index;
...
//!
const char *FlashProgrammer_CFV1::getProgramCapabilityNames(unsigned int actions) {
   unsigned index;
   static char buff[250] = "";
   static const char *actionTable[] = {
...
USBDM_ErrorCode FlashProgrammer_CFV1::convertTargetErrorCode(FlashDriverError_t rc) {
...
USBDM_ErrorCode FlashProgrammer_CFV1::initLargeTargetBuffer(uint8_t *buffer) {
   LOGGING;
   ...
   pFlashHeader->watchdogAddress = nativeToTarget32(device->getWatchdogAddress());
...
USBDM_ErrorCode FlashProgrammer_CFV1::initSmallTargetBuffer(uint8_t *buffer) {
   LOGGING_Q;
...
//!
USBDM_ErrorCode FlashProgrammer_CFV1::executeTargetProgram(uint8_t *pBuffer, uint32_t dataSize) {
   LOGGING;
...
//!
USBDM_ErrorCode FlashProgrammer_CFV1::determineTargetSpeed(void) {
   LOGGING;
...
//! @return error code see \ref USBDM_ErrorCode.
//!
USBDM_ErrorCode FlashProgrammer_CFV1::eraseFlash(void) {
   LOGGING;
   USBDM_ErrorCode rc = BDM_RC_OK;
...
//! @return error code see \ref USBDM_ErrorCode.
//!
USBDM_ErrorCode FlashProgrammer_CFV1::selectiveEraseFlashSecurity(void) {
   LOGGING;
   USBDM_ErrorCode rc = BDM_RC_OK;
...
//! @note - Assumes flash programming code has already been loaded to target.
//!
USBDM_ErrorCode FlashProgrammer_CFV1::partitionFlexNVM() {
   LOGGING;
...
//!
USBDM_ErrorCode FlashProgrammer_CFV1::checkUnsupportedTarget() {
   LOGGING;
   USBDM_ErrorCode rc;
   uint32_t targetSDID;
...
//!
USBDM_ErrorCode FlashProgrammer_CFV1::getTargetBusSpeed(unsigned long *busFrequency) {
   LOGGING;
...
//! @note Assumes the target device has already been opened & USBDM options set.
//!
USBDM_ErrorCode FlashProgrammer_CFV1::checkTargetUnSecured() {
   LOGGING;
   USBDM_ErrorCode rc = initialiseTarget();
   if (rc != PROGRAMMING_RC_OK)
      return rc;
...
//!
void FlashProgrammer_CFV1::deleteSecurityAreas(void) {
   securityAreaCount = 0;
}
...
//! @return error code see \ref USBDM_ErrorCode.
//!
USBDM_ErrorCode FlashProgrammer_CFV1::recordSecurityArea(const uint32_t address, const int size, const uint8_t *data) {
...
//!
void FlashProgrammer_CFV1::restoreSecurityAreas(FlashImagePtr flashImage) {
   LOGGING_Q;
...
//!
USBDM_ErrorCode FlashProgrammer_CFV1::setFlashSecurity(FlashImagePtr flashImage, MemoryRegionConstPtr flashRegion) {
   LOGGING;
...
//! @return error code see \ref USBDM_ErrorCode.
//!
USBDM_ErrorCode FlashProgrammer_CFV1::setFlashSecurity(FlashImagePtr flashImage) {
   LOGGING;
   // Process each flash region
   USBDM_ErrorCode rc = BDM_RC_OK;
...
//!
USBDM_ErrorCode FlashProgrammer_CFV1::doFlashBlock(FlashImagePtr flashImage,
...
//! @return error code see \ref USBDM_ErrorCode
//!
USBDM_ErrorCode FlashProgrammer_CFV1::applyFlashOperation(FlashImagePtr flashImage, FlashOperation flashOperation) {
   LOGGING;
...
//! @return error code see \ref USBDM_ErrorCode
//!
USBDM_ErrorCode FlashProgrammer_CFV1::doProgram(FlashImagePtr flashImage) {
   LOGGING;
...
//!
USBDM_ErrorCode FlashProgrammer_CFV1::doSelectiveErase(FlashImagePtr flashImage) {
   LOGGING;
...
//!
USBDM_ErrorCode FlashProgrammer_CFV1::doReadbackVerify(FlashImagePtr flashImage) {
   LOGGING;
...
//! @return error code see \ref USBDM_ErrorCode
//!
USBDM_ErrorCode FlashProgrammer_CFV1::doTargetVerify(FlashImagePtr flashImage) {
   LOGGING;
...
//! @return error code see \ref USBDM_ErrorCode
//!
USBDM_ErrorCode FlashProgrammer_CFV1::doVerify(FlashImagePtr flashImage) {
   LOGGING;
...
//! @return error code see \ref USBDM_ErrorCode
//!
USBDM_ErrorCode FlashProgrammer_CFV1::doBlankCheck(FlashImagePtr flashImage) {
   LOGGING;
...
//! @return error code see \ref USBDM_ErrorCode
//!
USBDM_ErrorCode FlashProgrammer_CFV1::doWriteRam(FlashImagePtr flashImage) {
   LOGGING;
...
//!       locations are ignored.
//!
USBDM_ErrorCode FlashProgrammer_CFV1::verifyFlash(FlashImagePtr flashImage,
...
//!
USBDM_ErrorCode FlashProgrammer_CFV1::programFlash(FlashImagePtr flashImage,
                                              CallBackT    progressCallBack,
                                              bool         doRamWrites) {
   LOGGING;
