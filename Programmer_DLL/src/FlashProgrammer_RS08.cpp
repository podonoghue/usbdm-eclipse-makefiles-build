...
USBDM_ErrorCode FlashProgrammer_RS08::resetAndConnectTarget(void) {
   ...
   if (rc != BDM_RC_OK) {
      ...
      log.print("failed reset with default method, retry with hardware\n");
      bdmInterface->connect();
...
//!
USBDM_ErrorCode FlashProgrammer_RS08::readTargetChipId(uint32_t *targetSDID, bool doInit) {
   LOGGING_E;
 ...
 *  @return error code, see \ref USBDM_ErrorCode
 */
USBDM_ErrorCode FlashProgrammer_RS08::massEraseTarget(bool resetTarget) {
   LOGGING;
...
USBDM_ErrorCode FlashProgrammer_RS08::blankCheckTarget() {
   ...
   for (memoryRegionIndex = 0;
        (memoryRegionPtr = device->getMemoryRegion(memoryRegionIndex));
        memoryRegionIndex++) {
      MemType_t memoryType = memoryRegionPtr->getMemoryType();
...
USBDM_ErrorCode FlashProgrammer_RS08::doImageCheck(FlashImagePtr flashImage) {
   LOGGING;
