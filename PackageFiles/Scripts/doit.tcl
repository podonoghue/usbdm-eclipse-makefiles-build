...
proc doit {} {
   ...
   wl $::DEMCR [expr $::DEMCR_VC_CORERESET|$::DEMCR_VC_HARDERROR]
...
proc massEraseTarget { } {
   ...
   set bdmCapability [getcap]
   if [expr ($bdmCapability & $::BDM_CAP_VDDCONTROL) != 0] {
      puts "Cycling Vdd"
      settargetvdd off
      after 1000
      settargetvdd on
   ...
   while { [expr $retry <= 20] }  {
         ...
         if [expr (($mdmApControl & $::MDM_AP_C_SYSTEM_RESET) != 0)] {
            puts "MDM_AP_C_SYSTEM_RESET success"
            break;
         }
         ;# Mass Erase + System Reset + Core Reset
   ...
   set flashEraseAcked [expr $mdmApStatus & $::MDM_AP_ST_MASS_ERASE_ACK]
   puts "mdmApStatus     = $mdmApStatus"
   puts "flashEraseAcked = $flashEraseAcked"
   
   set mdmApControl [rcreg $::MDM_AP_Control]
   set flashBusy [expr $mdmApControl & $::MDM_AP_C_MASS_ERASE]
   puts "mdmApControl    = $mdmApControl"
   puts "flashBusy       = $flashBusy"

   ;# Release the system reset
   set mdmApControl [expr $mdmApControl & ~$::MDM_AP_C_SYSTEM_RESET]
   wcreg $::MDM_AP_Control $mdmApControl

   ;# Wait for flash erase command complete
