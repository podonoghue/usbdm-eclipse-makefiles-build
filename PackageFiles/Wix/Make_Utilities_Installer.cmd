@echo off
cls
set VERSION=4_12_1_275
set VERSIONn=4.12.1.275
...
set REBUILT_WXS=DeviceData.wxs Stationery.wxs FlashImages.wxs UpdateSite.wxs
...
set LIGHT_DIRS=-b %SRC_DIR%\DeviceData -b %SRC_DIR%\Stationery -b %SRC_DIR%\FlashImages -b %SRC_DIR%\Examples -b %SRC_DIR%\UpdateSite

set HEAT_OPTIONS=-srd -ke -gg -sfrag -template fragment -sw5150
set MSI_FILE_UTILITY=USBDM_Utilities_%VERSION%_Win
set PATCH=patch_1_1
...
%LIGHT% %LIGHT_OPTIONS% %LIGHT_DIRS% -out %MSI_FILE_UTILITY% %WIX_BUILD_DIR%\*.wixobj

goto finish
:doPatch

%TORCH% -p -xi USBDM_4_9_0_Win.wixpdb %MSI_FILE_UTILITY%.wixpdb -out %PATCH%\diff.wixmst
