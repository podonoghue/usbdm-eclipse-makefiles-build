...
<h> Flash boot options (NV_FOPT)
   <e0> Boot ROM Options
      ...
      <i> Currently: KL03,KL17,KL27,KL43,KE15
      <0=> Disabled
      <1=> Enabled
   <o1> Boot Source Selection (FOPT.BOOTSRC_SEL)
      <i> These bits select the boot sources if FOPT.BOOTPIN_OPT = 1
      <info>BOOTSRC_SEL
      <0=> 0: Boot from ROM with BOOTCFG0/NMI pin low, or Boot from Flash with BOOTCFG0/NMI pin high
   ...
   <q2.0> Low power boot control (FOPT.LPBOOT)
      ...
      <i> Note: LPBOOT is only used on KE15
	   <info>LPBOOT
...
#define FOPT_MISC      (0xF)
#define FOPT_RESERVED  (0x30)

#if defined(NV_FOPT_BOOTSRC_SEL) && BOOT_ENABLE
#define FOPT_VALUE (FOPT_BOOTSRC|FOPT_RESERVED|FOPT_MISC)
#else
#define FOPT_VALUE (0xC2|FOPT_RESERVED|FOPT_MISC)
#endif
