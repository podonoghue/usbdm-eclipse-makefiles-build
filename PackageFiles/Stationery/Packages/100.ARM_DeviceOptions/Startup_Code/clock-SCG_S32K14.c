...
void clock_initialise(void) {
   ...
   // RUN mode configure
   SCG->RCCR =
         SCG_RCCR_SCS(scgMode_SystemPLL)| // Clock source = PLL
         SCG_RCCR_DIVCORE(2-1)|           // Core Clock  PLL/2  => 160/2 = 80 MHz
         SCG_RCCR_DIVBUS(2-1)|            // Bus Clock  Core/2  => 40 MHz
         SCG_RCCR_DIVSLOW(3-1);           // Slow Clock Core/3  => 26.7 MHz

   // VLPR mode configure
   SCG->VCCR =
         SCG_VCCR_SCS(scgMode_SlowIRC)|   // Clock source = SIRC ~ 2 MHz
         SCG_VCCR_DIVCORE(1-1)|           // Core Clock SIRC/1  => 2 MHz
         SCG_VCCR_DIVBUS(1-1)|            // Bus Clock  Core/1  => 2 MHz
...
void SystemCoreClockUpdate(void) {
   ...
   SystemBusClock  = (SystemCoreClock / (((SCG->CSR & SCG_CSR_DIVBUS_MASK) >> SCG_CSR_DIVBUS_SHIFT) + 1U));
}
