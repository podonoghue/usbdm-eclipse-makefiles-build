 ...
 * @param value   Value to modify (true/false)
 */
template <typename T>
inline void bitbandWrite(T &ref, const uint32_t bitNum, bool value) {
   uint32_t addr = (uint32_t)(&ref);
   const uint32_t mappedAddress = (addr&0xF0000000) + 0x02000000 + ((addr&0xFFFFF)*8*4) + (bitNum*4);
   *(volatile uint32_t *)(mappedAddress) = value?1:0;
}
