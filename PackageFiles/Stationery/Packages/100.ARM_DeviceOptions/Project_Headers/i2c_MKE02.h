...
enum PinselI2c {
   PinselI2c_Pta2_Pta3 = SIM_PINSEL_I2C0PS(0),
   PinselI2c_Ptb6_Ptb7 = SIM_PINSEL_I2C0PS(1),
};
...
public:
   ...
   static constexpr int numSignals  = 0;
   ...
   static constexpr uint32_t pinSelection = PinselI2c_Pta2_Pta3;

   // I2C SCL (clock) Pin
   using SclGpio = GpioA<3, USBDM::ActiveLow>; // Inactive is high

   // I2C SDA (data) Pin
   using SdaGpio = GpioA<2, USBDM::ActiveHigh>;
   ...
   static void initPCRs(uint32_t pinSelValue=pinSelection) {
      SIM->PINSEL = (SIM->PINSEL&~SIM_PINSEL_I2C0PS_MASK)|pinSelValue;
   }
...
public:
   ...
   virtual void busHangReset() {
      using sclGpio = typename Info::SclGpio;
      using sdaGpio = typename Info::SdaGpio;
