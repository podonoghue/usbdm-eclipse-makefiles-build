...
public:
   ...
   static const uint32_t SC_VALUE = TPM_SC_CMOD(1)|TPM_SC_PS(0);           //!< FTM->SC base value (clock selection & divider)
    ...
    */
   static void setPwmOutput(int period /* ticks */, Pwm_Mode mode=ftm_centreAlign) {
      ...
      *reinterpret_cast<volatile uint32_t*>(ftmClockReg) |= ftmClockMask;
      ...
      // Common registers
      reinterpret_cast<volatile TPM_Type*>(ftm)->SC      = TPM_SC_CMOD(0); // Disable FTM so register changes are immediate
      reinterpret_cast<volatile TPM_Type*>(ftm)->CNT     = 0;
      if (mode == ftm_centreAlign) {
         reinterpret_cast<volatile TPM_Type*>(ftm)->MOD  = period/2;
         // Centre aligned PWM with CPWMS not selected
         reinterpret_cast<volatile TPM_Type*>(ftm)->SC   = SC_VALUE|TPM_SC_CPWMS_MASK;
      }
      else {
         reinterpret_cast<volatile TPM_Type*>(ftm)->MOD  = period-1;
         // Left aligned PWM without CPWMS selected
         reinterpret_cast<volatile TPM_Type*>(ftm)->SC   = SC_VALUE;
      }
   }
   ...
   static void setDutyCycle(int dutyCycle) {
      reinterpret_cast<volatile TPM_Type*>(ftm)->CONTROLS[ftmChannel].CnSC = ftm_pwmHighTruePulses;

      if (reinterpret_cast<volatile TPM_Type*>(ftm)->SC&TPM_SC_CPWMS_MASK) {
         reinterpret_cast<volatile TPM_Type*>(ftm)->CONTROLS[ftmChannel].CnV  = (dutyCycle*reinterpret_cast<volatile TPM_Type*>(ftm)->MOD)/100;
      }
      else {
         reinterpret_cast<volatile TPM_Type*>(ftm)->CONTROLS[ftmChannel].CnV  = (dutyCycle*(reinterpret_cast<volatile TPM_Type*>(ftm)->MOD+1))/100;
      }
   }
   ...
   static void setPeriod(int period) {
      // Common registers
      reinterpret_cast<volatile TPM_Type*>(ftm)->SC = TPM_SC_PS(0); // Disable FTM so register changes are immediate

      if ((reinterpret_cast<volatile TPM_Type   *>(ftm)->SC&TPM_SC_CPWMS_MASK) != 0) {
         reinterpret_cast<volatile TPM_Type*>(ftm)->MOD = period/2;
         // Centre aligned PWM with CPWMS not selected
         reinterpret_cast<volatile TPM_Type*>(ftm)->SC  = SC_VALUE|TPM_SC_CPWMS_MASK;
      }
