...
public:
   enum {
      CONFIG_REG        = 0,     // See masks below
      EN_AA_REG         = 1,     // Enable auto acknowledgement data pipe n
      EN_RXADDR_REG     = 2,     // Enabled RX Addresses, Enable data pipe n
      SETUP_AW_REG      = 3,     // Setup of Address Widths (common for all data pipes)
      SETUP_RETR_REG    = 4,     // Setup of Automatic Retransmission
      RF_CH_REG         = 5,     // RF Channel
      RF_SETUP_REG      = 6,     // RF Setup Register
      STATUS_REG        = 7,     // Status Register. In parallel to the SPI command word applied on the MOSI pin,
      //                            the STATUS register is shifted serially out on the MISO pin.
      OBSERVE_TX_REG    = 8,     // Transmit observe register
      RPD_REG           = 9,     // Receive power detector
      ...
      //                             a PTX device with Enhanced ShockBurst™ enabled.
   ...
   //   static uint8_t SETUP_RETR(uint8_t ard, uint8_t arc) { return (uint8_t)(((ard)&0xF)<<4)|(((arc)&0xF)<<0); }
   //   static uint8_t SETUP_RETR_ARD(uint8_t x) { return (uint8_t)(((x)&0xF)<<4); }
   //   static uint8_t SETUP_RETR_ARC(uint8_t x) { return (uint8_t)(((x)&0xF)<<0); }

   /**
    *  Auto Retransmit delay in 250us steps (250-4000us)
    */
   constexpr static uint32_t SETUP_RETR_ARD(int x) {
      return (((x)&0xF)<<4);
   }
   /**
    * Auto Retransmit Count 0-15 retransmits (0 disables)
    */
   constexpr static uint32_t SETUP_RETR_ARC(int x) {
      return (((x)&0xF)<<0);
   }
   /**
    ...
    */
   constexpr static uint32_t RF_CH_CH(int x) {
      return (((x)&0x7F)<<0);
   }
...
#define STATUS_RX_P_NO(x) =  (((x)>>1)&0x7) // Data pipe number for the payload available for reading from RX_FIFO.
...
#define OBSERVE_TX_PLOS_CNT (((x)>>4)&0x0F)  // Count lost packets. The counter sticks at 15. The counter is reset by writing to RF_CH.
#define OBSERVE_TX_ARC_CNT  (((x)>>0)&0x0F)  // Count retransmitted packets. The counter is reset when transmission of a new packet starts.
   ...
   static constexpr uint8_t FIFO_STATUS_TX_REUSE  =  (1<<6); // Used for a PTX device. Pulse the rfce high for at least 10µs to reuse last transmitted pay-load.
...
protected:
   ...
   const NRFData *configData;

   NRF24L01Base(Spi *spi, const NRFData *configData=&DEFAULT_CONFIGURATION) : spi(spi) {
   }
...
public:
   ...
   uint8_t writeCommand(unsigned command, unsigned dataSize, const uint8_t data[]);
   uint8_t readCommand(unsigned command, unsigned dataSize, uint8_t data[]);
   uint8_t writeRegister(unsigned regNum, unsigned dataSize, const uint8_t data[]);
   uint8_t readRegister(unsigned regNum, unsigned dataSize, uint8_t data[]);
...
class NRF24L01_T : public NRF24L01Base {
public:
   NRF24L01_T(Spi *spi, const NRFData *configData=&DEFAULT_CONFIGURATION) : NRF24L01Base(spi, configData) {

      spi->setSpeed(10000000 /* Hz */);

   #ifndef MCU_MKL25Z4
      spi->setCTAR0Value(SPI_CTAR_FMSZ(8-1)|SPI_CTAR_PCSSCK(0)|SPI_CTAR_PASC(0)|SPI_CTAR_PDT(0)|SPI_CTAR_CSSCK(0)|SPI_CTAR_ASC(0)|SPI_CTAR_DT(8));
   #endif

      // Enable CE port pin clock
