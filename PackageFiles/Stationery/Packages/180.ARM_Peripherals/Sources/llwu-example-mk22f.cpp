/*
 ============================================================================
 * @file    llwu-example-mk22f.cpp (180.ARM_Peripherals/Sources/)
 * @brief   Basic C++ demo
 *
 ...
 *  - ClockConfig_BLPE_4MHz   For VLPR (Core/Bus = 4MHz, Flash = 1kHz)
 *
...
void runTest(
   ...
   switch(test) {
      ...
      case STOP:  testStopMode(SmcStopMode_NormalStop,         SmcLowLeakageStopMode_LLS3);  break;
      case VLPS:  testStopMode(SmcStopMode_VeryLowPowerStop,   SmcLowLeakageStopMode_LLS3);  break;
...
void help() {
   console.write(
      "\n\n"
      "WAIT  - Wait mode (enter from RUN)\n"
      "VLPW  - Very low power wait (enter from VLPR)\n"
      "STOP  - Stop mode (enter from RUN)\n"
      "VLPS  - Very low power Stop (enter from VLPR)\n"
...
int main() {
   ...
   for(;;) {
      ...
      char prompt[100];
      char *pPtr=prompt;
      pPtr = FormattedIO::strcpy(pPtr, "\rE - Execute test (");
      pPtr = FormattedIO::strcpy(pPtr, Smc::getSmcStatusName());
      pPtr = FormattedIO::strcpy(pPtr, ", ");
      pPtr = FormattedIO::strcpy(pPtr, Mcg::getClockModeName());
      pPtr = FormattedIO::strcpy(pPtr, "@");
      pPtr = FormattedIO::ultoa(::SystemCoreClock, pPtr);
      pPtr = FormattedIO::strcpy(pPtr, enablePin?", Pin":"     ");
      pPtr = FormattedIO::strcpy(pPtr, enableTimer?", Timer":"       ");
      pPtr = FormattedIO::strcpy(pPtr, ", ");
      pPtr = FormattedIO::strcpy(pPtr, TestNames[test]);
      pPtr = FormattedIO::strcpy(pPtr, ") :   ");
